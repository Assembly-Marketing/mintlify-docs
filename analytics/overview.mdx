...
Refreshing stats for a LinkedIn company page requires the connected member to still be an admin of that Page. If the member who connected the profile loses their admin role on LinkedIn, **Refresh Stats** fails with the error "This LinkedIn Page is no longer administered by the connected account. Reconnect it with a current admin to refresh stats." Ordinal also marks the profile as needing reconnection.
